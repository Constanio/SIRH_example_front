...
<template>
  <header class="h-20 flex items-center justify-between px-10 bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-[50]">
    <!-- Left: Search Bar -->
    <div class="flex-1 max-w-xl">
      <div class="relative group">
        ...
        <input 
          ...
          class="w-full pl-11 pr-16 py-2.5 bg-slate-100/50 border-2 border-transparent focus:border-indigo-500/20 focus:bg-white rounded-2xl text-sm font-medium text-slate-700 outline-none transition-all"
        />
        <div class="absolute inset-y-0 right-4 flex items-center gap-1 pointer-events-none">
          <kbd class="px-1.5 py-0.5 bg-white border border-slate-200 rounded-md text-[10px] font-black text-slate-400 shadow-sm">
    ...
    <div class="flex items-center gap-6">
      ...
      <div class="flex items-center gap-2 pr-6 border-r border-slate-100">
        ...
        <button @click="isDark = !isDark" class="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all">
      ...
      <div class="relative">
        <button 
          ...
          class="flex items-center gap-3 p-1.5 pr-4 hover:bg-slate-50 rounded-2xl transition-all border border-transparent hover:border-slate-100 group"
        >
          <div class="relative">
          ...
          <div class="text-left hidden md:block">
            <p class="text-xs font-black text-slate-900 leading-none mb-1">{{ auth.user?.prenom }} {{ auth.user?.nom }}</p>
        ...
        >
          <div v-if="showUserMenu" class="absolute right-0 mt-3 w-64 bg-white rounded-3xl shadow-2xl shadow-slate-200 border border-slate-100 py-3 z-[60]">
            ...
            <div class="px-2 space-y-1">
              <button @click="router.push('/profil')" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all">
                <User class="w-4 h-4" />
                Mon Profil
              </button>
              <button class="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all">
            ...
            <div class="mt-3 pt-3 border-t border-slate-50 px-2">
              <button @click="logout" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-rose-500 hover:bg-rose-50 rounded-xl transition-all">
